...
namespace zadania
{
    class Program
    {
        ...
        {
            Console.Write("wybierz zadanie 1-8 : ");
            int x;
        ...
        {
            int[] vecA=new int[3],vecB = new int[3];
            for (int i = 1; i < 3; i++)
            {
                for (int j = 0; j < vecA.Length; j++)
                {
                    Console.Write($"Podaj {j+1} zmienną w {i} wektorze");
                    if (i==1)
                    { vecA[j] = int.Parse(Console.ReadLine()); }
                    else
                    { vecB[j] = int.Parse(Console.ReadLine()); }
                }
            }
            int[] vecC = vectorMultiply(vecA, vecB);
            
            Console.WriteLine($"Iloczyn wektorowy VecA[{vecA[0]};{vecA[1]};{vecA[2]}] * VecB[{vecB[0]};{vecB[1]};{vecB[2]}] = [{vecC[0]};{vecC[1]};{vecC[2]}]");
        }
        ...
        static int[] vectorMultiply(int[] vecA,int[] vecB)
        {
            int[] VecC = new int[3];
            VecC[0] =(vecA[1]*vecB[2])-(vecA[2]*vecB[1]);
            VecC[1] =-1*((vecA[0]*vecB[2])-(vecA[2]*vecB[0]));
